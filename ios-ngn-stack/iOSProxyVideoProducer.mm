...
- (void)startVideoCapture{
    ...
    dispatch_queue_t queue = dispatch_queue_create("org.weicall.producer.captureoutput", NULL);
	...
	if([NSThread currentThread] != [NSThread mainThread]){// From worker thread?
...
- (void)timerBlankPacketsTick:(NSTimer*)timer{
	...
	if((_producer = (tmedia_producer_t *)tsk_object_ref((void*)_mProducer->getWrappedPlugin()))){
		...
		switch (TMEDIA_PRODUCER(_producer)->video.chroma) {
			...
			case tmedia_chroma_none:
			case tmedia_chroma_rgb24:		// will be stored as bgr24 on x86 (little endians) machines; e.g. WindowsPhone7
			case tmedia_chroma_bgr24:		// used by windows consumer (DirectShow) - 
            case tmedia_chroma_rgb565le:	// (used by both android and wince consumers)
            case tmedia_chroma_rgb565be:
            case tmedia_chroma_nv21: // Yuv420 SP (used by android producer)
            case tmedia_chroma_yuv422p:
            case tmedia_chroma_yuv420p: // Default
            default:
                break;
		}
		if(buffer_size<sizeof(kBlankPacketBuffer)){
			NgnLog(@"Sending Blank packet number %d", mBlankPacketsSent);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
	...
	if(CVPixelBufferLockBaseAddress(pixelBuffer, 0) == kCVReturnSuccess){
		...
		// http://code.google.com/p/WeiCall/issues/detail?id=27&q=UInt8
...
-(int) start{
	...
	if(!_mSenderQueue){
		_mSenderQueue = dispatch_queue_create("org.weicall.producer.sender", NULL);
