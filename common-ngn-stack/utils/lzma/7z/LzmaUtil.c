...
int LzmaUtil(char mode, const char* infile, const char* outFile)
{
    ...
    encodeMode = (mode == 'e' || mode == 'E');
    if (!encodeMode && mode != 'd' && mode != 'D')
        return -1;
    
    {
        ...
        if (t4 != 4 || t8 != 8) {
            //PrintError(rs, "Incorrect UInt32 or UInt64");
            return -1;            
        }
    }
