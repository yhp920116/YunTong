...
public:
	...
	int OnDialogEvent(const DialogEvent* _e){
        ...
        const char* _subserv = _sipMesssage ? const_cast<SipMessage*>(_sipMesssage)->getSipHeaderValue("Contact") : "";
        ...
        NSString* subserv = [NgnStringUtils toNSString:_subserv];
		
		TSK_DEBUG_INFO("OnDialogEvent(%s, %ld, '%s')", _phrase, _sessionId, _subserv?_subserv:"");
        if (_subserv) {
            TSK_FREE(_subserv);
            _subserv = NULL;
        }
	...
	int OnInviteEvent(const InviteEvent* _e) { 
				...
				if(_message && (((ngnSipSession = [NgnAVSession getSessionWithId: _sessionId]) != nil) || ((ngnSipSession = [NgnMsrpSession getSessionWithId: _sessionId]) != nil))){
					if(const_cast<SipMessage*>(_message)->getRequestType() == tsip_INFO){
						...
						if(_content && _content_type){
							...
							if([contentType isEqualToString:kContentDeviceInfo]){
        ...
        [eargs autorelease];    //gary for memory  leak
		[pool release];
		return 0; 
	}
	...
	int OnMessagingEvent(const MessagingEvent* _e) { 
		...
		switch (_type) {
			...
			case tsip_i_message:
			{
                ...
                if (_ctype != NULL)
                {
                    if((!_content || !_content_length) && !strstr(_ctype, "authfail")){
                        TSK_DEBUG_ERROR("Invalid MESSAGE");
                        [ngnSession reject];
				...
				if([ctype caseInsensitiveCompare: kContentType3gppSMS] == NSOrderedSame){
                    ...
                    char* _callid = const_cast<SipMessage*>(_message)->getSipHeaderValue("i");
                    //NSString* callid = [[NSString alloc] initWithCString:_callid encoding:NSASCIIStringEncoding];
					eargs = [[NgnMessagingEventArgs alloc] 
                             ...
                             andCallId: [NgnStringUtils toNSString: _callid]];
...
-(NgnSipService*)init{
	if((self = [super init])){
        ...
        mInfoService = [[[NgnEngine sharedInstance] getInfoService] retain];
	}
	return self;
}
...
-(NSString*)getDefaultIdentity{
	...
	if(self->sipDefaultIdentity){
		...
		return [[NgnEngine sharedInstance].infoService getInfoValueForkeyWithDecrypt:IDENTITY_IMPU];
	}
}
...
-(BOOL)registerIdentity{
	...
	if(self->sipStack && (!self->sipRegSession || !self->sipRegSession.connected)){//registration terminated but stack still not destroyed => create new stack
		...
		if(self->sipStack.state != STACK_STATE_STOPPED && self->sipStack.state != STACK_STATE_STOPPING){
            NgnLog(@"-------------sip stop  %d", self->sipStack.state);
	...
	sipPreferences.impi = [mInfoService getInfoValueForkeyWithDecrypt:IDENTITY_IMPI];
	sipPreferences.impu = [mInfoService getInfoValueForkeyWithDecrypt:IDENTITY_IMPU];
    sipPreferences.deviceToken = [mConfigurationService getStringWithKey:SECURITY_DEVICE_TOKEN];
    
	NgnNSLog(TAG, @"realm='%@', impu='%@', impi='%@',token='%@'", sipPreferences.realm, sipPreferences.impu, sipPreferences.impi, sipPreferences.deviceToken);
	
    int maketType = [mConfigurationService getIntWithKey:GENERAL_MAKET_TYPE];
	if (sipStack == nil) {
		sipStack = [[NgnSipStack alloc] initWithSipCallback:_mSipCallback andRealmUri:sipPreferences.realm andIMPIUri:sipPreferences.impi andIMPUUri:sipPreferences.impu andDeviceToken: sipPreferences.deviceToken andMaketType:maketType];
	} else {
	...
	// set the Password
    NSString* pwd = [mInfoService getInfoValueForkeyWithDecrypt:IDENTITY_PASSWORD];
	[sipStack setPassword: pwd];
	// Set AMF
	...
	// Set Proxy-CSCF
	sipPreferences.pcscfHost = [mConfigurationService getStringWithKey:NETWORK_PCSCF_REG_HOST];
	sipPreferences.pcscfPort = [mConfigurationService getIntWithKey:NETWORK_PCSCF_REG_PORT];
