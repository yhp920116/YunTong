...
#undef FREE
	...
	for(ifr = ifc.ifc_req; ifr && !tsk_strnullORempty(ifr->ifr_name); ifr++){
...
#define ADVANCE(x, n) (x += ROUNDUP((n)->sa_len))
    ...
    // Vincent fix system crash - if code goto bail before declare ifaddr, e.g. in line 533.
    if (ifaddr) {
        freeifaddrs(ifaddr);//gary for memory leak
        ifaddr = tsk_null;
    }
