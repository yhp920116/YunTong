...
tsk_size_t tsip_transport_send_raw(const tsip_transport_t* self, const struct sockaddr* to, void* data, tsk_size_t size)
{
	tsk_size_t ret = 0;
    ...
    TSK_DEBUG_INFO("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Send >>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n\
%s>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n", (char*)data);
    if (self->stack->security.enable_encrpt) {
        EncryptPacket((char*)data, size);
    }
