...
parse_buffer:
	...
	*/
    char* data_ptr = TSK_BUFFER_DATA(transport->buff_stream);
    if (transport->stack->security.enable_encrpt) {
        DecryptPacket(data_ptr, strlen(data_ptr));
    }
    TSK_DEBUG_INFO("\n<<<<<<<<<<<<<<<<<<<<<< Receive T <<<<<<<<<<<<<<<<<<<<<<\n\
%s<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n", data_ptr ? data_ptr : "");
    
	tsk_ragel_state_init(&state, data_ptr, endOfheaders + 4/*2CRLF*/);
...
static int tsip_transport_layer_dgram_cb(const tnet_transport_event_t* e)
{
    ...
    if (transport->stack->security.enable_encrpt) {
        DecryptPacket(data_ptr, data_size);
    }
    TSK_DEBUG_INFO("\n<<<<<<<<<<<<<<<<<<<<<< Receive <<<<<<<<<<<<<<<<<<<<<<\n\
%s<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n", data_ptr ? data_ptr : "");
