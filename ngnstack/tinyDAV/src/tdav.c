...
#if HAVE_SPEEX_DSP &&  HAVE_SPEEX_JB && 0
...
#	endif
	...
	/* === Register Audio/video JitterBuffer === */ //gary test speakup jb
#if HAVE_SPEEX_DSP && HAVE_SPEEX_JB && 0 
...
int tdav_codec_get_priority(tdav_codec_id_t* codec_id)
{
	static int count = sizeof(__codecs)/sizeof(tdav_codec_decl_t);
	int i = 0;
    int j = 0;
	for(i = 0; i<count; ++i){
        tdav_codec_decl_t codec_decl;
        codec_decl = __codecs[i];
        if (_tdav_codec_is_supported(codec_decl.id, *codec_decl.plugin) && tmedia_codec_plugin_is_registered(*codec_decl.plugin)) {
            codec_id[j] = __codecs[i].id;
            j++;
        }
	}
	return j;
...
const char* tdav_codec_get_codec_name(tdav_codec_id_t codec) {
    switch(codec) {
        case tdav_codec_id_amr_nb_oa:
            return "amr_nb_oa";
        case tdav_codec_id_amr_nb_be:
