...
bool MediaSessionMgr::getSessionCodec(codecInfoDef& codec, twrap_media_type_t media) const {
    if (media != twrap_media_audio){
		TSK_DEBUG_ERROR("Invalid media type");
		return false;
	}
	...
	if(m_pWrappedMgr){
		tmedia_type_t _media = _get_media_type(media);
		tmedia_session_t* session = tmedia_session_mgr_find(m_pWrappedMgr, _media);
		if(session && TDAV_SESSION_AUDIO(session)->encoder.codec){
            TSK_DEBUG_INFO("getSessionCodec %s",TDAV_SESSION_AUDIO(session)->encoder.codec->name);
            codec.name = TDAV_SESSION_AUDIO(session)->encoder.codec->name;
            codec.rate = TDAV_SESSION_AUDIO(session)->encoder.codec->plugin->rate;
            return true;
		} else {
            TSK_DEBUG_ERROR("getSessionCodec: failed");
            if (!session) {
                TSK_DEBUG_ERROR("getSessionCodec: session");
            } else {
                TSK_DEBUG_ERROR("getSessionCodec: codec");
            }
        }
	} else {
		TSK_DEBUG_ERROR("Invalid state");
