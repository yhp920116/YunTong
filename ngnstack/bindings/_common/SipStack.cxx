...
bool SipStack::setEncryptSecAgree(bool enabled)
{
	tsk_bool_t _enable = enabled;
	return (tsip_stack_set(m_pHandle,
                           TSIP_STACK_SET_SECAGREE_ENCRYPT(_enable),
                           TSIP_STACK_SET_NULL()) == 0);
}
...
int SipStack::getCodecPriority(tdav_codec_id_t* codec_id)
{
    if (codec_id)
        return tdav_codec_get_priority(codec_id);
    
    return 0;
}

const char* SipStack::getCodecName(tdav_codec_id_t codec_id) {
    return tdav_codec_get_codec_name(codec_id);
}
