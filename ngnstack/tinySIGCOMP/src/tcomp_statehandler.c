...
static int pred_find_compartment_by_id(const tsk_list_item_t *item, const void *id)
{
	if(item && item->data){
		...
		uint64_t r1 = compartment->identifier;
        uint64_t r2 = *((uint64_t*)id);
		return r1 > r2 ? (int)1 : (r1 < r2 ? (int)-1 : (int)0);
	}
	return -1;
}
...
void tcomp_statehandler_handleResult(tcomp_statehandler_t *statehandler, tcomp_result_t **lpResult)
{
	...
	*/
    lpCompartment = tcomp_statehandler_getCompartment(statehandler, (*lpResult)->compartmentId);
	if(lpCompartment){
