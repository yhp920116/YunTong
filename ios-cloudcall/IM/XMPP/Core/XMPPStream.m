...
/////////////////////
-(NSData*) buildSendData:(NSData*)data {
    if (!data)
        return nil;
        
#if IMEncrypt_XMPP_Enable
    EncryptPacket((char*)[data bytes], [data length]);
    
    xmppHeaderInfo header;
    header.msg_resv = htonl(0xFFFFFFFF);
    header.msg_len  = htonl([data length]);
    
    NSData *sendData = [[NSData alloc] initWithBytes:&header length:sizeof(xmppHeaderInfo)];
    NSMutableData *dataArry = [[NSMutableData alloc] init];
    [dataArry appendData:sendData];
    [dataArry appendData:data];
...
-(NSData*) buildSendDataWithString:(NSString*)string {
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    return [self buildSendData:data];
}
////////////////////
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (state != STATE_XMPP_DISCONNECTED)
		{
			...
			else
			{
				...
				NSData *outgoingData = [self buildSendDataWithString:termStr];
...
- (void)sendStartTLSRequest
{
    ...
    NSData *outgoingData = [self buildSendDataWithString:starttls];
    numberOfBytesSent += [outgoingData length];
...
- (BOOL)registerWithPassword:(NSString *)password error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSData *outgoingData = [self buildSendDataWithString:outgoingStr];
        numberOfBytesSent += [outgoingData length];
...
- (void)sendOpeningNegotiation
{
	...
	if (![self didStartNegotiation])
	{
        ...
        NSData *outgoingData = [self buildSendDataWithString:s1];
        numberOfBytesSent += [outgoingData length];
    ...
    NSData *outgoingData = [self buildSendDataWithString:s2];
    numberOfBytesSent += [outgoingData length];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
...
#if IMEncrypt_XMPP_Enable
    NSMutableData* dataArray = [[NSMutableData alloc] init];
    ...
    if (remainPCData)
    {
        [fullData appendData:remainPCData];
        [fullData appendData:data];
        pcData = (char*)[fullData bytes];
        datalen = [fullData length];
        remainPCData = nil;
    }
    else
    {
        pcData = (char*)[data bytes];
        datalen = [data length];
    }

    NSLog(@"1 RECV: 11 len %d ", datalen);
    while (datalen) {
        if (datalen < sizeof(xmppHeaderInfo)) {
            //NSLog(@"1 RECV:remvain data length:%d  dataLen:%d", len, datalen);
            remainPCData = [NSData dataWithBytes:pcData length:datalen];
            break;
        }
        
        int len = datalen;
        xmppHeaderInfo* header = (xmppHeaderInfo*)pcData;
        int msg_resv = ntohl(header->msg_resv);
        if (msg_resv == 0xFFFFFFFF) {
            len = ntohl(header->msg_len);
            if (len > (datalen - sizeof(xmppHeaderInfo))) {
                //NSLog(@"1 RECV:remvain data length:%d  dataLen:%d", len, datalen);
                remainPCData = [NSData dataWithBytes:pcData length:datalen];
                break;
            }
            datalen -= sizeof(xmppHeaderInfo);
            pcData += sizeof(xmppHeaderInfo);

            DecryptPacket(pcData, len);        
            
            NSLog(@"1 RECV: len %d %d", len, datalen);
            //NSLog(@"1 RECV: 2 %s", pcData);
        } else {
            NSLog(@"1 RECV: Not decrypt data!!");
            
            NSLog(@"1 RECV: 3 %s", pcData);
        }
        
        NSData* recvData = [NSData dataWithBytes:pcData length:len];
        XMPPLogRecvPre(@"1 RECV: %@", recvData);
        
        numberOfBytesReceived += [recvData length];
        [dataArray appendData:recvData];
	...
	[parser parseData:dataArray];
#else    
    XMPPLogRecvPre(@"1 RECV: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"2 RECV: %@", [root compactXMLString]);
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"3 RECV: %@", [element compactXMLString]);
...
- (void)keepAlive
{
	...
	if (state == STATE_XMPP_CONNECTED)
	{
		...
		{
			NSData* outgoingData = [self buildSendData:[NSData dataWithData:keepAliveData]];
			numberOfBytesSent += [outgoingData length];
			[asyncSocket writeData:outgoingData
			           withTimeout:TIMEOUT_XMPP_WRITE
			                   tag:TAG_XMPP_WRITE_STREAM];
